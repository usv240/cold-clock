...
- Show one continuous live action: create case, trigger excursion, human review, sandbox fulfillment, delivery and receipt.
- Show a deliberate failure path and both proof suites; do not rely only on slides.
- Show the `.run.app` address bar, Cloud Run revision, one Firestore case/wake transition, Vertex AI Gemini recording evidence and Cloud Trace.
