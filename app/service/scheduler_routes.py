...
def build_scheduler_router(store,scheduler):
 ...
 def scan(authorization:str|None=Header(default=None)):
  ...
  rows=scheduler.dispatch_due(lambda wake:executor.execute(wake,trigger=identity));return {"ok":True,"identity":identity,"dispatched":[row.wake_id for row in rows],"dead_letters":[row.wake_id for row in scheduler.dead_letters]}
 return router
