...
| 0:20–0:45 | Click **Run unattended**. Live Gemini reads the synthetic package (5 exact-quoted fields); Gemma screens the text; Embedding routes it; the **ADK agent assembles the packet through three scoped tools and the verifier accepts it**. The run stops at the human gate | Medicine evidence tab: verified fields, injection screen "clean"; Review tab: "ADK agent packet accepted · 3 scoped tool calls · 6 values verified", `AI disposition: none` |
| 0:45–1:10 | **You** click Record human disposition and type a real name, disposition and rationale — the one decision the system refuses to make. Reservation and dispatch then happen by themselves | Review dialog, then journey rail advancing with no further clicks |
| 1:10–1:25 | Stop. Point at the **Durable wakes** panel: `courier_status_poll · pending · due …` and the line "Cloud Scheduler scanned 12s ago (verified Google OIDC)". Say: "Nobody clicks from here." | Wake panel, autonomy rail `0 continue clicks` |
