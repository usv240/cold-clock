...
Two more pieces made it feel like an agent rather than a script. A Google ADK `LlmAgent` assembles the pharmacist packet, but it can only see the case through three read-only tools, and a verifier compares every value it writes to what the tools returned — an invented number or a "this is safe" sentence throws the whole packet away. And one Pub/Sub utility-outage message fans out to every enrolled household in the grid area; fifteen minutes later each case is judged by its own background wake from its own readings.
